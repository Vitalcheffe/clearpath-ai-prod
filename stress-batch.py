#!/usr/bin/env python3
"""ClearPath AI — 1000 Bot Stress Test (Batch mode with incremental saves)"""
...
if os.path.exists(OUT):
    try:
        ...
        print(f"Resuming from {done} existing results", flush=True)
    except:
        all_results = []
else:
    done = 0
...
start_idx = done
print(f"Total bots: {len(all_bots)}, Already done: {done}, Remaining: {len(all_bots)-done}", flush=True)

for batch_start in range(start_idx, len(all_bots), BATCH):
    batch_end = min(batch_start + BATCH, len(all_bots))
    batch = all_bots[batch_start:batch_end]
    print(f"\n── Batch {batch_start}-{batch_end-1} ──", flush=True)
    bt0 = time.time()
    
    for i, (btype, inp, city, expected) in enumerate(batch):
        ...
        all_results.append({"type":btype, "score":sc, "issues":iss, "expected":expected,
                        ...
                        "input": inp[:60], "categories": [c.get("label","?") for c in res.get("categories",[])] if not res.get("_err") else []})
        
        if (global_idx+1) % 50 == 0:
            el = time.time()-t0_global
            avg = sum(r["score"] for r in all_results)/len(all_results)
            print(f"  [{global_idx+1}/1000] {el:.0f}s | avg:{avg:.0f} | errs:{sum(r['err'] for r in all_results)}", flush=True)
    ...
    print(f"  Batch done in {batch_time:.0f}s, saved {len(all_results)} results", flush=True)
...
for r in all_results:
    if r["type"]=="normal" and not r["err"]:
        ...
        if r["expected"].split()[0] in " ".join(r.get("categories",[])):
...
total=len(all_results); avg=sum(r["score"] for r in all_results)/total
pr=sum(1 for r in all_results if r["score"]>=60)/total*100
crisis_n=sum(1 for r in all_results if r["type"]=="crisis"); crisis_ok=sum(1 for r in all_results if r["crisis"] and r["type"]=="crisis")
vague_n=sum(1 for r in all_results if r["type"]=="vague"); vague_ok=sum(1 for r in all_results if r["vague"] and r["type"]=="vague")
bart=sum(1 for r in all_results if r["src"]=="bart"); errs=sum(r["err"] for r in all_results)

print(f"\n{'='*80}")
print(f"GLOBAL SCORE: {avg:.0f}/100 | Pass: {pr:.0f}% | Crises: {crisis_ok}/{crisis_n} | Vagues: {vague_ok}/{vague_n} | BART: {bart}/{total} | Errs: {errs}")
print(f"{'='*80}")

# Save final summary alongside results
final = {
    "summary": {
        "global_score": round(avg, 1),
